...
module ``uhppote_api``. Importing the integration package instead would execute
``custom_components/ha_accesscontrol/__init__.py``, which pulls in Home
Assistant; the protocol layer is deliberately free of that dependency so these
tests run anywhere.

conftest.py is imported before any test module, so ``import uhppote_api`` works
